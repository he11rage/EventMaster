...
def home(request):
    ...
    hall_numbers = list(range(1, 49))
    random.shuffle(hall_numbers)

    for i, event in enumerate(events):
        if i < len(hall_numbers):
            event.hall_number = hall_numbers[i]
        else:
            event.hall_number = "Зал ещё не назначен"
    ...
    if request.method == "POST":
        try:
            ...
            if not title or not description or not city:
                return JsonResponse(
                    {"error": "Недостаточно данных для создания события"}, status=400
                )
            ...
            try:
                ...
                return JsonResponse(
                    {"error": f'Категория "{category_name}" не найдена'}, status=400
                )
            ...
            return JsonResponse(
                {"message": "Мероприятие успешно добавлено!"}, status=200
            )
        except Exception as e:
    ...
    data = [
        {
            ...
            "end": (
                reg.event.end_datetime.isoformat() if reg.event.end_datetime else None
            ),
        }
...
def register_for_event(request, event_id):
    ...
    if already_registered:
        return JsonResponse(
            {"success": False, "message": "Вы уже записались на это мероприятие."}
        )
